...
if (strUrlPage=='externaldata') {
  document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
  <div id="sigma-container" class="border border-solid vh-100">${strUrlPage}</div>
  `
}
else {
  document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
...
else if (strUrlPage=='externaldata') {
    graph.addNode("1", { label: "External data 1", x: 0, y: 0, size: 20, color: "green" });
    graph.addNode("2", { label: "External data 2", x: 1, y: 1, size: 30, color: "orange" });
    graph.addEdge("2", "1", { size: 10, color: "purple" });
}
else{
