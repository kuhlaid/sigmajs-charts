...
- run `yarn add sigma graphology ejs graphology-types` (you should be in the `sigma-graph-esm` app directory before running this command so the packages do not get committed to the code repository); this will also update yarn; the `ejs` package is used to serve .ejs template files
